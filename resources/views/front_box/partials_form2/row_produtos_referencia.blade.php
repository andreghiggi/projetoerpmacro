<div class="row mt-1 products product-line-{{$code}}">
	...
	<div class="col-md-5 text-left cart-data">
		...
		<span class="price">R$ {{ __moeda($valor )}}</span><br>
	...
	<input type="hidden" class="quantidade" name="quantidade[]" value="{{ $quantidade }}">
	<input type="hidden" class="valor_unitario" name="valor_unitario[]" value="{{ __moeda($item->valor_unitario) }}">
	<input type="hidden" class="subtotal_item" name="subtotal_item[]" value="{{ __moeda($valor) }}">
